"""Portfolio v1 endpoints — RESTful-compliant routes.

Changes from legacy API:
- GET /holdings?account_id=X → GET /accounts/:account_id/holdings
- POST /trades/close → PUT /holdings/:holding_id (close as state transition)
...
@router.get("/accounts")
def list_accounts(view: str | None = Query(None, description="View type: summary")):
    """List all accounts. Use ?view=summary for aggregated portfolio data."""
    from backend.db import get_conn
    ...
    if view == "summary":
        # Return account summary with portfolio metrics
    ...
    # Default view: just account metadata
    return {
        "accounts": [
            {
                "id": r["id"],
...
# --------------- Holdings Management (RESTful nested resources) ---------------

@router.get("/accounts/{account_id}/holdings")
def list_account_holdings(account_id: str):
    """List all holdings for a specific account.

    Merges live quotes (same as account summary) into each row's ``currentPrice`` in
    the JSON response only — does **not** persist prices to the database.
    """
    from backend.db import get_conn
...
def create_account_holding(account_id: str, req: CreateHoldingRequest):
    """Create a new holding for a specific account."""
    from datetime import datetime, timezone as tz
    from backend.db import get_conn
    ...
    conn.execute(
        """INSERT INTO holdings
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            holding_id, account_id, req.asset_type, req.ticker.upper(),
...
@router.put("/holdings/{holding_id}/close")
def close_holding(holding_id: str, req: CloseTradeRequest):
    """Close a holding and record the trade. This is a state transition (PUT)."""
    from datetime import datetime, timezone as tz
    from backend.db import get_conn
