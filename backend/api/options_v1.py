"""Options & market data v1 endpoints — RESTful-compliant routes.

Changes from legacy API:
- GET /quote?ticker=X → GET /tickers/:ticker/quote
- GET /price/:ticker → GET /tickers/:ticker/price (more consistent naming)
- GET /sell-put-analysis/:ticker → GET /tickers/:ticker/sell-put-analysis
- GET /theta-comparison/:ticker → GET /tickers/:ticker/theta-comparison
- GET /volatility/:ticker → GET /tickers/:ticker/volatility
- GET /earnings/:ticker → GET /tickers/:ticker/earnings
- GET /expirations/:ticker → GET /tickers/:ticker/expirations
- GET /options-chain/:ticker → GET /tickers/:ticker/options-chains/:expiration
"""
...
@router.get("/tickers/{ticker}/price")
...
def get_ticker_quote(
    ...
    """Get current quote for any market. Lightweight single-ticker lookup."""
    import ssl
...
def get_ticker_expirations(ticker: str):
...
@router.get("/tickers/{ticker}/options-chains")
def get_ticker_options_chain(
    ticker: str,
...
@router.get("/tickers/{ticker}/volatility")
def get_ticker_volatility(ticker: str):
...
@router.get("/tickers/{ticker}/sell-put-analysis")
def get_ticker_sell_put_analysis(
    ticker: str,
...
@router.get("/tickers/{ticker}/theta-comparison")
def get_ticker_theta_comparison(
    ticker: str,
    strike: float = Query(..., description="Strike price"),
):
...
@router.get("/tickers/{ticker}/earnings")
...
@router.get("/exchange-rates/usd-cny")
def get_usd_cny_exchange_rate():
    """Get current USD/CNY exchange rate (cached 1h)."""
    import ssl
    import time
    import urllib.request

    cache = getattr(get_usd_cny_exchange_rate, "_cache", None)
    ...
    get_usd_cny_exchange_rate._cache = {"ts": time.time(), "data": result}
    return result
